			...
			<div class="lenvy-container pb-8 flex items-end gap-4">
				...
				<?php if ( $logo_id ) : ?>
					<div class="shrink-0 w-14 h-14 bg-white rounded flex items-center justify-center p-1.5 shadow-sm mb-0.5">
						<?php echo wp_get_attachment_image( $logo_id, [ 56, 56 ], false, [ // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				<div>
					<h1 class="text-3xl md:text-4xl font-serif italic text-white leading-tight">
						<?php echo $brand_name; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped — already esc_html'd ?>
					</h1>
		...
		<div class="lenvy-container flex items-center gap-4">
			...
			<?php if ( $logo_id ) : ?>
				<div class="shrink-0 w-12 h-12 bg-white border border-neutral-200 rounded flex items-center justify-center p-1">
					<?php echo wp_get_attachment_image( $logo_id, [ 48, 48 ], false, [ // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						'class' => 'w-full h-full object-contain',
						'alt'   => esc_attr( $term->name ),
					] ); ?>
				</div>
			...
			<div>
				<h1 class="text-2xl font-serif italic text-neutral-900">
					<?php echo $brand_name; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</h1>
...
<?php
// ── 2. Metadata bar (country + website — only if at least one is set) ──────────
...
<main id="primary" class="py-8 lg:py-12">
	<div class="lenvy-container">
		...
		<div class="flex gap-8 mt-6">
			...
			<?php
			get_template_part( 'template-parts/shop/filter-sidebar', null, [
			...
			<div class="flex-1 min-w-0">
				...
				<?php
				// ── Brand description below grid (SEO + brand storytelling) ───────
				if ( $description ) :
					?>
					<div class="mt-16 pt-10 border-t border-neutral-100 prose prose-sm max-w-none text-neutral-600">
						<?php
						// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped — ACF WYSIWYG returns safe HTML.
...
<?php
get_template_part( 'template-parts/shop/filter-drawer', null, [
	'hide_brand_filter' => true,
] );
?>
