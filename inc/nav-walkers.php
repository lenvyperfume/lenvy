...
class Lenvy_Primary_Nav_Walker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $data_object, $depth = 0, $args = null, $current_object_id = 0 ) {
		...
		if ( $depth === 0 ) {
			...
			$link_class  = 'flex items-center gap-1 py-1 text-sm font-medium transition-colors duration-150';
			$link_class .= $is_current ? ' text-black underline underline-offset-4' : ' text-neutral-700 hover:text-black';
			$output     .= '<a href="' . $url . '"' . $target . ' class="' . esc_attr( $link_class ) . '">';
			$output     .= esc_html( $title );

			if ( $has_children ) {
				$output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="w-3.5 h-3.5 shrink-0 transition-transform duration-150 group-hover:rotate-180 group-focus-within:rotate-180" aria-hidden="true" focusable="false"><polyline points="6 9 12 15 18 9"/></svg>';
			}
			$output .= '</a>';
			...
			$link_class  = 'block px-4 py-2 text-sm transition-colors duration-150';
			$link_class .= $is_current ? ' text-black font-medium bg-neutral-50' : ' text-neutral-700 hover:bg-neutral-50 hover:text-black';
